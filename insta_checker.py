        ...
        for content in notification:
            ...
            try:
                if 'Сергей' in sender and content != 'Stories':
                    await bot.send_message(chat_id=telegram_chat_id, text=textwrap.dedent(message))
                else:
                    await bot.send_message(chat_id=private_tg_id, text=textwrap.dedent(message))
            except RetryAfter as e:
                logger.warning(f"Flood control exceeded. Retry in {e.timeout} seconds.")
                await asyncio.sleep(e.timeout)
                await send_notifications({sender: [content]})
            except Exception as e:
                logger.error(f"Error occurred during send_notifications: {e}")
    ...
    for sender, notification in notifications.items():
        for content in notification:
            ...
            try:
                ...
                logger.warning(f"Flood control exceeded. Retry in {e.timeout} seconds.")
                await asyncio.sleep(e.timeout)
                await send_notifications({sender: [content]})  # Повторная попытка отправки сообщения
            except Exception as e:
                logger.error(f"Error occurred during send_notifications: {e}")
